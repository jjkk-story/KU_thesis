...
data64 <- read.csv("C:/Users/samsung/Desktop/(NBS) 53-67차 데이터/data64.csv")

data64_1 <- select(data64, -c(ID, SQ1, AGE, Q2_1, Q3, Q6, WT))
data64_1 <- data64_1[c(12,1,2,3,4,13,6,7,8,9,10,11,5)]

for(i in 1:ncol(data64_1)){
  data64_1[,i] <- as.factor(data64_1[,i])
}

data64_te <- subset(data64_1, Q4 == 98 | Q4 == 99)
data64_tr <- subset(data64_1, Q4 != 98 & Q4 != 99)

data64_tr$Q4 <- droplevels(data64_tr$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답)
all <- multinom(Q4 ~ 0 + ., data=data64_tr)
pred <- predict(all, data64_te)

### 의사결정나무
tree <- rpart(Q4 ~ ., data = data64_tr, method = "class", cp = -1)
pred_tree <- predict(tree, data64_te, type = "class")
...
rf <- randomForest(Q4 ~ ., data = data64_tr, ntree = 100, mtry = 12)
pred_rf <- predict(rf, data64_te, type = "class")

############# +안철수

data64_te3 <- subset(data64_1, Q4 == 98 | Q4 == 99 | Q4 == 4)
data64_tr3 <- subset(data64_1, Q4 != 98 & Q4 != 99 & Q4 != 4)

data64_tr3$Q4 <- droplevels(data64_tr3$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답 & 안철수)
all3 <- multinom(Q4 ~ 0 + ., data=data64_tr3)
pred3 <- predict(all3, data64_te3)

### 의사결정나무
tree3 <- rpart(Q4 ~ ., data = data64_tr3, method = "class", cp = -1)
pred_tree3 <- predict(tree3, data64_te3, type = "class")
...
rf3 <- randomForest(Q4 ~ ., data = data64_tr3, ntree = 100, mtry = 12)
pred_rf3 <- predict(rf3, data64_te3, type = "class")

#############

### 다항 로지스틱 회귀분석
data64_te$Q4 <- pred
result <- rbind(data64_tr, data64_te)
...
data64_te3$Q4 <- pred3
result3 <- rbind(data64_tr3, data64_te3)
...
result3 <- cbind(result3, data64$WT)
...
### 의사결정나무
data64_te$Q4 <- pred_tree
result_tree <- rbind(data64_tr, data64_te)
...
result_tree <- cbind(result_tree, data64$WT)
...
data64_te3$Q4 <- pred_tree3
result_tree3 <- rbind(data64_tr3, data64_te3)
...
result_tree3 <- cbind(result_tree3, data64$WT)
...
### 랜덤포레스트
data64_te$Q4 <- pred_rf
result_rf <- rbind(data64_tr, data64_te)
...
result_rf <- cbind(result_rf, data64$WT)
...
data64_te3$Q4 <- pred_rf3
result_rf3 <- rbind(data64_tr3, data64_te3)
...
result_rf3 <- cbind(result_rf3, data64$WT)
