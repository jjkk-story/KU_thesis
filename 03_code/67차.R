...
data67 <- read.csv("C:/Users/samsung/Desktop/(NBS) 53-67차 데이터/data67.csv")

data67_1 <- select(data67, -c(ID, SQ1, AGE, Q2_1, Q8_1, Q1, Q4, WT))
data67_1 <- data67_1[c(12,1,2,6,7,13,4,5,8,9,10,11,3)]

for(i in 1:ncol(data67_1)){
  data67_1[,i] <- as.factor(data67_1[,i])
}

data67_te <- subset(data67_1, Q2 == 98 | Q2 == 99)
data67_tr <- subset(data67_1, Q2 != 98 & Q2 != 99)

data67_tr$Q2 <- droplevels(data67_tr$Q2)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답)
all <- multinom(Q2 ~ 0 + ., data=data67_tr)
pred <- predict(all, data67_te)

### 의사결정나무
tree <- rpart(Q2 ~ ., data = data67_tr, method = "class", cp = -1)
pred_tree <- predict(tree, data67_te, type = "class")
...
rf <- randomForest(Q2 ~ ., data = data67_tr, ntree = 100, mtry = 12)
pred_rf <- predict(rf, data67_te, type = "class")

############# +안철수

data67_te3 <- subset(data67_1, Q2 == 98 | Q2 == 99 | Q2 == 4)
data67_tr3 <- subset(data67_1, Q2 != 98 & Q2 != 99 & Q2 != 4)

data67_tr3$Q2 <- droplevels(data67_tr3$Q2)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답 & 안철수)
all3 <- multinom(Q2 ~ 0 + ., data=data67_tr3)
pred3 <- predict(all3, data67_te3)

### 의사결정나무
tree3 <- rpart(Q2 ~ ., data = data67_tr3, method = "class", cp = -1)
pred_tree3 <- predict(tree3, data67_te3, type = "class")
...
rf3 <- randomForest(Q2 ~ ., data = data67_tr3, ntree = 100, mtry = 12)
pred_rf3 <- predict(rf3, data67_te3, type = "class")

#############

### 다항 로지스틱 회귀분석
data67_te$Q2 <- pred
result <- rbind(data67_tr, data67_te)
...
result <- cbind(result, data67$WT)
colnames(result)[15] <- "WT"
result <- result[,-1]
a <- sum(result$WT)
s1 <- data.frame(result %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
s1 <- data.frame(matrix(s1[,2], nrow = 1))

data67_te3$Q2 <- pred3
result3 <- rbind(data67_tr3, data67_te3)
...
result3 <- cbind(result3, data67$WT)
colnames(result3)[15] <- "WT"
result3 <- result3[,-1]
a <- sum(result3$WT)
s2 <- data.frame(result3 %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
s2 <- data.frame(matrix(s2[,2], nrow = 1))

### 의사결정나무
data67_te$Q2 <- pred_tree
result_tree <- rbind(data67_tr, data67_te)
...
result_tree <- cbind(result_tree, data67$WT)
colnames(result_tree)[15] <- "WT"
result_tree <- result_tree[,-1]
a <- sum(result_tree$WT)
s3 <- data.frame(result_tree %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
s3 <- data.frame(matrix(s3[,2], nrow = 1))

data67_te3$Q2 <- pred_tree3
result_tree3 <- rbind(data67_tr3, data67_te3)
...
result_tree3 <- cbind(result_tree3, data67$WT)
colnames(result_tree3)[15] <- "WT"
result_tree3 <- result_tree3[,-1]
a <- sum(result_tree3$WT)
s4 <- data.frame(result_tree3 %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
s4 <- data.frame(matrix(s4[,2], nrow = 1))

### 랜덤포레스트
data67_te$Q2 <- pred_rf
result_rf <- rbind(data67_tr, data67_te)
...
result_rf <- cbind(result_rf, data67$WT)
colnames(result_rf)[15] <- "WT"
result_rf <- result_rf[,-1]
a <- sum(result_rf$WT)
s5 <- data.frame(result_rf %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
s5 <- data.frame(matrix(s5[,2], nrow = 1))

data67_te3$Q2 <- pred_rf3
result_rf3 <- rbind(data67_tr3, data67_te3)
...
result_rf3 <- cbind(result_rf3, data67$WT)
colnames(result_rf3)[15] <- "WT"
result_rf3 <- result_rf3[,-1]
a <- sum(result_rf3$WT)
s6 <- data.frame(result_rf3 %>% group_by(Q2) %>%
                   summarise(freq = sum(WT)/a))
