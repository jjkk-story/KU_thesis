...
data60 <- read.csv("C:/Users/samsung/Desktop/(NBS) 53-67차 데이터/data60.csv")

data60_1 <- select(data60, -c(ID, SQ1, AGE, Q2_1, Q3, Q6, WT))
data60_1 <- data60_1[c(11,1,2,3,4,12,6,7,8,9,10,5)]

for(i in 1:ncol(data60_1)){
  data60_1[,i] <- as.factor(data60_1[,i])
}

data60_te <- subset(data60_1, Q4 == 98 | Q4 == 99)
data60_tr <- subset(data60_1, Q4 != 98 & Q4 != 99)

data60_tr$Q4 <- droplevels(data60_tr$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답)
all <- multinom(Q4 ~ 0 + ., data=data60_tr)
pred <- predict(all, data60_te)

### 의사결정나무
tree <- rpart(Q4 ~ ., data = data60_tr, method = "class", cp = -1)
pred_tree <- predict(tree, data60_te, type = "class")
...
rf <- randomForest(Q4 ~ ., data = data60_tr, ntree = 100, mtry = 11)
pred_rf <- predict(rf, data60_te, type = "class")

############# +안철수

data60_te3 <- subset(data60_1, Q4 == 98 | Q4 == 99 | Q4 == 4)
data60_tr3 <- subset(data60_1, Q4 != 98 & Q4 != 99 & Q4 != 4)

data60_tr3$Q4 <- droplevels(data60_tr3$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답 & 안철수)
all3 <- multinom(Q4 ~ 0 + ., data=data60_tr3)
pred3 <- predict(all3, data60_te3)

### 의사결정나무
tree3 <- rpart(Q4 ~ ., data = data60_tr3, method = "class", cp = -1)
pred_tree3 <- predict(tree3, data60_te3, type = "class")
...
rf3 <- randomForest(Q4 ~ ., data = data60_tr3, ntree = 100, mtry = 11)
pred_rf3 <- predict(rf3, data60_te3, type = "class")

#############

### 다항 로지스틱 회귀분석
data60_te$Q4 <- pred
result <- rbind(data60_tr, data60_te)
...
result <- cbind(result, data60$WT)
colnames(result)[14] <- "WT"
result <- result[,-1]
a <- sum(result$WT)
...
data60_te3$Q4 <- pred3
result3 <- rbind(data60_tr3, data60_te3)
...
result3 <- cbind(result3, data60$WT)
colnames(result3)[14] <- "WT"
result3 <- result3[,-1]
a <- sum(result3$WT)
...
### 의사결정나무
data60_te$Q4 <- pred_tree
result_tree <- rbind(data60_tr, data60_te)
...
result_tree <- cbind(result_tree, data60$WT)
colnames(result_tree)[14] <- "WT"
...
data60_te3$Q4 <- pred_tree3
result_tree3 <- rbind(data60_tr3, data60_te3)
...
result_tree3 <- cbind(result_tree3, data60$WT)
colnames(result_tree3)[14] <- "WT"
...
### 랜덤포레스트
data60_te$Q4 <- pred_rf
result_rf <- rbind(data60_tr, data60_te)
...
result_rf <- cbind(result_rf, data60$WT)
colnames(result_rf)[14] <- "WT"
...
data60_te3$Q4 <- pred_rf3
result_rf3 <- rbind(data60_tr3, data60_te3)
...
result_rf3 <- cbind(result_rf3, data60$WT)
colnames(result_rf3)[14] <- "WT"
