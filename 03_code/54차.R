...
data54 <- read.csv("C:/Users/samsung/Desktop/(NBS) 53-67차 데이터/data54.csv")

data54_1 <- select(data54, -c(ID, SQ1, AGE, Q2_1, Q3, Q5, WT))
data54_1 <- data54_1[c(12,1,2,3,4,13,6,7,8,9,10,11,5)]

for(i in 1:ncol(data54_1)){
  data54_1[,i] <- as.factor(data54_1[,i])
}

data54_te <- subset(data54_1, Q4 == 98 | Q4 == 99)
data54_tr <- subset(data54_1, Q4 != 98 & Q4 != 99)

data54_tr$Q4 <- droplevels(data54_tr$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답)
all <- multinom(Q4 ~ 0 + ., data=data54_tr)
pred <- predict(all, data54_te)

### 의사결정나무
tree <- rpart(Q4 ~ ., data = data54_tr, method = "class", cp = -1)
pred_tree <- predict(tree, data54_te, type = "class")
...
rf <- randomForest(Q4 ~ ., data = data54_tr, ntree = 100, mtry = 12)
pred_rf <- predict(rf, data54_te, type = "class")

############# +안철수

data54_te3 <- subset(data54_1, Q4 == 98 | Q4 == 99 | Q4 == 4)
data54_tr3 <- subset(data54_1, Q4 != 98 & Q4 != 99 & Q4 != 4)

data54_tr3$Q4 <- droplevels(data54_tr3$Q4)

### 다항 로지스틱 회귀분석 (없다 & 모름/무응답 & 안철수)
all3 <- multinom(Q4 ~ 0 + ., data=data54_tr3)
pred3 <- predict(all3, data54_te3)

### 의사결정나무
tree3 <- rpart(Q4 ~ ., data = data54_tr3, method = "class", cp = -1)
pred_tree3 <- predict(tree3, data54_te3, type = "class")
...
rf3 <- randomForest(Q4 ~ ., data = data54_tr3, ntree = 100, mtry = 12)
pred_rf3 <- predict(rf3, data54_te3, type = "class")

#############

### 다항 로지스틱 회귀분석
data54_te$Q4 <- pred
result <- rbind(data54_tr, data54_te)
...
data54_te3$Q4 <- pred3
result3 <- rbind(data54_tr3, data54_te3)
...
result3 <- cbind(result3, data54$WT)
...
### 의사결정나무
data54_te$Q4 <- pred_tree
result_tree <- rbind(data54_tr, data54_te)
...
result_tree <- cbind(result_tree, data54$WT)
...
data54_te3$Q4 <- pred_tree3
result_tree3 <- rbind(data54_tr3, data54_te3)
...
result_tree3 <- cbind(result_tree3, data54$WT)
...
### 랜덤포레스트
data54_te$Q4 <- pred_rf
result_rf <- rbind(data54_tr, data54_te)
...
result_rf <- cbind(result_rf, data54$WT)
...
data54_te3$Q4 <- pred_rf3
result_rf3 <- rbind(data54_tr3, data54_te3)
...
result_rf3 <- cbind(result_rf3, data54$WT)
